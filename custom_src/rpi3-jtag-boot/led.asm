...
// first: sanitize argument and write to message buffer in memory
  ...
  mov argument, #1
arg_ok:
  str argument, [message, #0x18]
// reset request code, request/response size, and pin number
  mov argument, #0
  str argument, [message, #0x4]
  str argument, [message, #0x10]
  mov argument, #130
  str argument, [message, #0x14]
  .unreq argument
  .unreq message
